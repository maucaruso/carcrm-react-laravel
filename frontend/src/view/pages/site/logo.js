...
export default function Logo() {
  ...
  const _handleUpload = (event) => {
    ...
    dispatch(uploadLogo(body)).then(
      (res) => res && setState({ isUpload: null })
    );
  };
        ...
        <div className="d-flex flex-column align-items-center mt-5">
          {app.logo ? (
            <>
              <img
                alt=""
                style={state.isDeleted ? { opacity: 0.5 } : {}}
                className="img-fluid"
                src={
                  rootUrl + "thumb/logo/" + app.logo + "?u=" + app.id + "&h=150"
                }
              />
