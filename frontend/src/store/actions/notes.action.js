...
export const index = (query, isLoadMore) => (dispatch) => {
  return HttpAuth.get("/notes?" + new URLSearchParams(query)).then(
    (res) =>
...
export const store = (data) => (dispatch) => {
  return HttpAuth.post("/notes", data).then(
    (res) => typeof res !== "undefined" && dispatch(storeResponse(res.data))
  );
};
...
export const update = (data) => (dispatch) => {
  return HttpAuth.put("/notes/" + data.id, data).then((res) => {
...
export const destroy = (id) => (dispatch) => {
  return HttpAuth.delete("/notes/" + id).then(
