...
export const index = (query) => (dispatch) => {
  return HttpAuth.get("/units?" + new URLSearchParams(query)).then(
...
export const store = (data) => (dispatch) => {
  ...
  return HttpAuth.post("/units", data).then((res) => {
...
export const show = (id) => (dispatch) => {
  return HttpAuth.get("/units/" + id).then(
    (res) => typeof res !== "undefined" && dispatch(indexResponse(res.data))
  );
};
...
export const update = (data) => (dispatch) => {
  ...
  return HttpAuth.put("/units/" + data.id, data).then((res) => {
    dispatch(changeLoading({ open: true }));
...
export const destroy = (id) => dispatch => {
  return HttpAuth.delete('/units/' + id).then(res => typeof res !== "undefined" && dispatch(destroyResponse(id)));
}
